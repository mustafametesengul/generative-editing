...
class Cli(BaseSettings):
    ...
    seed: int = 0
    candidates: int = Field(1, ge=1, description="Seeded candidates to generate and verify")
    cpu_offload: bool = Field(False, description="Offload FLUX components to CPU to reduce VRAM")
    debug_dir: Path | None = Field(None, description="Optional directory for decomposition masks")
    ...
    def cli_cmd(self) -> None:
        ...
        editor = Flux2KleinEditor(cpu_offload=self.cpu_offload) if self.editor == "flux" else MockWeatherEditor()
        pipeline = WeatherEditingPipeline(decomposer, editor, candidates=self.candidates)
        ...
        report = result.metrics.model_dump() | {"selected_seed": result.seed, "passed_preservation": result.passed}
