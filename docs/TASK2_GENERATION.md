...
Material response — diffuse cloud illumination on facades, wet-asphalt reflection, accumulation restricted to receptive surfaces — is delegated to the editor: the minimal prompts leave it to the model's own weather prior, and only snow carries explicit material clauses. The verifier confirms the response happened where licensed. In production, a learned material-and-surface-orientation head would sharpen both the prompt policy and the target-specific verification thresholds, for example distinguishing upward-facing accumulation from vertical facades.
